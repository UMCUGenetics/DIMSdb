...
from .models import DIMSRun, DIMSResults, HMDB, Patient, Sample, DIMSResultsAdductLink
...
def get_dimsrun_samples(name: str, id: str):
    with Session(engine) as session:
        query = select(DIMSResults, HMDB).join(DIMSResultsAdductLink,
                                               onclause=DIMSResults.uuid == DIMSResultsAdductLink.run_uuid) \
            .join(HMDB, onclause=HMDB.uuid == DIMSResultsAdductLink.hmdb_uuid).where(DIMSResults.run_name == name,
                                                                                     DIMSResults.sample_id == id)
        result = session.exec(query).all()
    return result
...
def get_sample_zscoremax_result(name: str, id: str, zscoremax: float):
    with Session(engine) as session:
        query = select(DIMSResults, HMDB).join(DIMSResultsAdductLink,
                                               onclause=DIMSResults.uuid == DIMSResultsAdductLink.run_uuid,
                                               isouter=True) \
            .join(HMDB, onclause=HMDB.uuid == DIMSResultsAdductLink.hmdb_uuid, isouter=True).where(
    ...
    with Session(engine) as session:
        if iden == "iden":
            query = select(DIMSResults, HMDB) \
                .join(DIMSResultsAdductLink, onclause=DIMSResults.uuid == DIMSResultsAdductLink.run_uuid, isouter=True) \
                .join(HMDB, onclause=HMDB.uuid == DIMSResultsAdductLink.hmdb_uuid, isouter=True) \
                .where(col(DIMSResults.run_name).in_(run_names),
                       col(DIMSResultsAdductLink.hmdb_uuid).in_(
                           select(DIMSResultsAdductLink.hmdb_uuid) \
                               .join(DIMSResults, onclause=DIMSResultsAdductLink.run_uuid == DIMSResults.uuid,
                                     isouter=True) \
                               .where(col(DIMSResults.run_name).in_(run_names),
                                      col(DIMSResults.sample_id).in_(samples),
                                      col(DIMSResultsAdductLink.hmdb_uuid).is_not(None),
                                      or_(DIMSResults.z_score <= zscoremin, DIMSResults.z_score >= zscoremax))
                       )
                ).distinct()
        elif iden == "not_iden":
                ...
                .where(col(DIMSResults.run_name).in_(run_names),
                       col(DIMSResultsAdductLink.hmdb_uuid).in_(
                           select(DIMSResultsAdductLink.hmdb_uuid) \
                               ...
                               .where(col(DIMSResults.run_name).in_(run_names),
                                      col(DIMSResults.sample_id).in_(samples),
                                      col(DIMSResultsAdductLink.hmdb_uuid).is_(None),
                                      or_(DIMSResults.z_score <= zscoremin, DIMSResults.z_score >= zscoremax))
                       )
                ).distinct()
        elif iden == "all":
