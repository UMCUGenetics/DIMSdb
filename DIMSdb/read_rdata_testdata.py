...
def get_samples_run(file_pos, file_neg):
    ...
    for file in [file_pos, file_neg]:
        ...
        sample_ids.extend([col_name for col_name in identified_df.columns
...
def get_data_run(file):
    ...
    merged_df = merged_df.drop(
        ["fq.best", "fq.worst", "nrsamples", "mzmin.pgrp",
         "mzmax.pgrp", "iso_HMDB", "theormz_HMDB", "avg.int",
         "assi_noise", "theormz_noise", "avg.ctrls", "sd.ctrls"], axis=1)

    # melt and pivot the table to get for each sample a row with the intensity and zscore
    merged_df_melt = merged_df.melt(id_vars=['mzmed.pgrp', 'ppmdev', 'assi_HMDB', 'HMDB_code', 'row_hash'],
                                    var_name='Sample_Type',
                                    value_name='Intensity')
    merged_df_melt[['Sample', 'Type']] = merged_df_melt['Sample_Type'].str.extract(r'(.+?)(?:_(Zscore))?$')
    merged_df_pivot = merged_df_melt.pivot(index=['mzmed.pgrp', 'ppmdev', 'assi_HMDB',
                                                  'HMDB_code', 'row_hash', 'Sample'],
                                           columns='Type', values='Intensity').reset_index()
    # rename columns
    merged_df_pivot.columns = ['mzmed.pgrp', 'ppmdev', 'assi_HMDB', 'HMDB_code',
                               'row_hash', 'Sample', 'Intensity', 'Zscore']
...
def get_dimsresults_dict(dimsresults_df, polarity, run_name):
    ...
    dimsresults_df.columns = ['m_z', 'ppm_dev', 'HMDB_code', 'row_hash',
                              'sample_id', 'intensity', 'z_score']
...
def add_dimsresults(file, polarity, run_name, size_chunk):
    ...
    with Session(engine) as session:
        for i in range(0, len(dimsresults_dict), size_chunk):
            session.execute(
                insert(DIMSResults),
                dimsresults_dict[i:i + size_chunk]
            )
            session.commit()
...
def add_new_hmdb(hmdb_info):
    ...
    hmdb_info['hmdb_key'], hmdb_info['hmdb_id'], hmdb_info['sec_hmdb_id'] = hmdb_info['HMDB_code'], hmdb_info['HMDB_code'], hmdb_info['HMDB_code']
