...
def parse_settings_file(file, runname, session):
...
def parse_rdata(file, runname, session):
...
def main():
    ...
    for run_name in run_names:
        with Session(engine) as session:
            ...
            parse_settings_file(settings_file, run_name, session)
            file_neg = path_name + run_name + '/outlist_identified_negative.RData'
            print(file_neg)
            parse_rdata(file_neg, run_name, session)
            file_pos = path_name + run_name + '/outlist_identified_positive.RData'
            print(file_pos)
            parse_rdata(file_pos, run_name, session)
