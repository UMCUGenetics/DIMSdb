...
class DIMSResultsHMDBLink(SQLModel, table=True):
    hmdb_uuid: Optional[int] = Field(
        default=None, foreign_key="hmdb.uuid", primary_key=True
    )
    run_uuid: Optional[int] = Field(
        default=None, foreign_key="dimsresults.uuid", primary_key=True
    )
