...
run_single_patients = function(patient_ID, path_monsters, path_plots, path_output, make_plots){
  ...
  CreateWorkbookSheet <- function(wb, DT, filelist, imagesize_multiplier = 2, columns = NULL, path_plots){
    ...
    if (is.null(columns)) columns <- rev(length(runs) : 1)
    if (make_plots == 1){
      for (irun in columns){
        sample_png <- NULL
        writeLines(paste("Start", runs[irun], filelist))
        plotdir <- paste0(path_plots, "/", runs[irun], "/plots/adducts")
        for (irow in rows_DT:1) {
          ...
          if (!length(H_code)==0 && is.na(H_code)==FALSE) {
            ...
            if(is.null(sample_png)){
              ...
              setColWidths(wb, filelist, cols = irun, widths = cell_dim[2]/20)
            }
            tryCatch({
              insertImage(wb, filelist, file_png, startRow = irow+1, startCol = irun, height = cell_dim[1], width = cell_dim[2], units = "px")
            }, error=function(e){writeLines(paste("image not available for", H_code, "in run", runs[irun]))})
            if(irow%%100 == 0){writeLines(paste("at row:",irow))}
          }
        }
    ...
    if (ncol(printit) == 1) {
      ...
      setColWidths(wb, filelist, cols = which_cols_start:which_cols_end, widths = 16)
  ...
  DBS_projectinfo_DT <- as.data.table(read.xlsx(xls_file, sheet = "Overzicht patiënten"))
  Patient_col_name <- grep("Patient", names(DBS_projectinfo_DT), value = TRUE)
  names(DBS_projectinfo_DT)[names(DBS_projectinfo_DT) == Patient_col_name] <- "Patient.code"
  DBS_projectinfo_DT[, Patient.code := gsub(" ","", Patient.code)]
  NumDBS_col_name <- grep("DBS", names(DBS_projectinfo_DT), value = TRUE)
  NumDBS_col_name <- NumDBS_col_name[nchar(grep("DBS", names(DBS_projectinfo_DT), value = TRUE)) < 6]
  names(DBS_projectinfo_DT)[names(DBS_projectinfo_DT) == NumDBS_col_name] <- "nr.DBS"
  ...
  DBS_DT_overzichtpatient <- na.omit(DBS_DT_overzichtpatient) # remove rows with NA (has no full run data)
  
  #get run info
  DBS_projectinfo_DT <- as.data.table(read.xlsx(xls_file, sheet = "Overzicht runs"))
  select_cols <- c("Patient.code", "Sample.code", "Single.Patients.Run")
  DBS_DT_overzichtruns <- DBS_projectinfo_DT[, ..select_cols]
  DBS_DT_overzichtruns <- na.omit(DBS_DT_overzichtruns) # remove rows with NA (has no full run data)
  DBS_DT_overzichtruns <- DBS_DT_overzichtruns[!grepl('/|DBS missing', DBS_DT_overzichtruns$Sample.code),] # remove rows with weird sample codes (with / and "DBS missing") (are not directly correlated to the runs) 
    
  #combine two dataframes to get full overview of patients and runs
  DBS_DT_full <- join(DBS_DT_overzichtpatient, DBS_DT_overzichtruns, type="full")
  DBS_DT_full <- na.omit(DBS_DT_full) # remove rows with NA (has no full run data)
  ...
  for(patient in patient_ID){
    patient_number_of_DBS <- as.integer(patient_number_of_DBS) + as.integer(DBS_DT_overzichtpatient[which(DBS_DT_overzichtpatient[,Patient.code] == patient), nr.DBS])
    # which runs is this patient in?
    runs <- c(runs, unique(subset(DBS_DT_full,Patient.code==patient,select="Single.Patients.Run")))
    runs <- unique(as.list(runs))
  }
  
 
  # write information about number of dbs in .txt file (to later check if all runs were found)
  ##write.table(patient_number_of_DBS, file = paste0(path_output,"/", patient_ID,"_", patient_number_of_DBS, "_numberDBS.txt"), row.names = FALSE, col.names = FALSE, quote = FALSE)
  
  # +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
  # Select file(s) where analysis results for 'Patient' are -----------------
  # +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
  
  filenames <- list.files(path=path_monsters, pattern='^RES_DBS.*.xlsx', full.names=FALSE)
  ...
  full_filenames <- unique(grep(paste(toMatch,collapse="|"), filenames, value=TRUE))
  ...
  for( i in 1:length(full_filenames)){
    DT <- as.data.table(read.xlsx(paste0(path_monsters, full_filenames[[i]])))
    colnames_in_file <- vapply(strsplit(names(DT),".", fixed = TRUE), `[`, 1, FUN.VALUE=character(1))
    if(length(DT_list) == 0){
      patient_DT_subset <- DT[ , grepl(paste0(patient_ID, "$", collapse = "|"), colnames_in_file, perl = TRUE) | names(DT) %in% c("HMDB_code","name","descr"), with = FALSE]
      DT_list[[i]] <- patient_DT_subset
    } else {
      patient_DT_subset <- DT[ , grepl(paste0(patient_ID, "$", collapse = "|"), colnames_in_file, perl = TRUE) | names(DT) %in% c("HMDB_code"), with = FALSE]
      DT_list[[i]] <- patient_DT_subset
    }
  }
  
  # Create a single dataframe with all columns of the patient
  patient_DT <- data.table(join_all(DT_list, by = "HMDB_code", type = "full"))
  
  setcolorder(patient_DT, c(grep("Zscore", names(patient_DT), invert = TRUE, value = TRUE), grep("Zscore", names(patient_DT), value = TRUE)))
  ...
  absent <- as.character(patient_DT[rowSums(is.na(patient_DT[,grep(paste(patient_ID, collapse = "|"), names(patient_DT)), with = FALSE])) > 0, "HMDB_code"])
  if(length(absent) == 0 | absent == "character(0)"){
    writeLines("No HMDB codes (partly) unique")
  } else {
    writeLines(paste(c("HMDB codes (partly) unique to some files: \n", absent,"\n"), collapse=" "))
    write.table(absent, file = paste0(path_output,"/", paste(patient_ID, collapse = "_"), "_unique_HMDB_codes.txt"), row.names = FALSE, col.names = FALSE, quote = FALSE)
  }
  ...
  DT_criteria <- data.frame("DBS"=c(1,2,3,4,5), "Elevated Threshold"=Threshold_pos, "Decreased Threshold"=Threshold_neg)
  ...
  Z_scores_DT <- patient_DT[, .SD,  .SDcols = c("HMDB_code", Z_score_cols)]
  ...
  if (number_of_DBS < 4){
    HMDB_elevated <- Z_scores_DT[elevated_DBS == number_of_DBS, HMDB_code]
    HMDB_decreased <- Z_scores_DT[decreased_DBS == number_of_DBS, HMDB_code]
  ...
  patient_DT_elevated <- patient_DT_elevated %>%
    ...
    mutate(all_HMDBs = paste(HMDB_code, collapse = " | "), all_descrs = paste(descr, collapse = " | "), all_names = paste(name, collapse = " | ")) %>% 
  ...
  modifyBaseFont(wb, fontSize = 10, fontColour = "#000000", fontName = "Arial Narrow")
